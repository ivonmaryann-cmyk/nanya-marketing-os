...
class OrderTaskStatusTests(unittest.TestCase):
    def test_entry_completion_and_reply_are_independent(self):
        for created, completed, replied, label in (
            (False, False, False, '待生成模板'),
            (False, False, True, '待生成模板'),
            (True, False, False, '待录单'),
            (True, False, True, '待录单'),
            (True, True, False, '录单完成待回复'),
            (True, True, True, '录单完成已回复'),
        ):
            with self.subTest(created=created, completed=completed, replied=replied):
                cursor = MagicMock()
                cursor.__enter__.return_value.execute.return_value.fetchone.return_value = {'id': 1} if replied else None
                with patch.object(service, '_entry_progress', return_value={'created': created, 'completed': completed, 'label': '待批量料号查询'}), patch.object(service, 'db_cursor', return_value=cursor):

...
def main() -> None:
    with TemporaryDirectory() as temp_dir_name:
        ...
        try:
            ...
            loaded_agent_rules = agent_rules.load_transcode_agent_rules(agent_version)
            assert len(loaded_agent_rules) == 207
            assert any(rule.get("规则ID") == "TAR-CYB-20260721-001" for rule in loaded_agent_rules)
            loaded_semantic_rules = semantic_rules.load_transcode_semantic_rules(semantic_version)
            ...
            assert semantic_rules.validate_transcode_semantic_rule_version(semantic_version)["rule_count"] == 76

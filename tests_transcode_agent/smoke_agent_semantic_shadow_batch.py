...
def main() -> None:
    ...
    with TemporaryDirectory() as temp_dir_name:
        ...
        try:
            ...
            blocked_count = int(enforce_summary.get("正式证据门禁拦截") or 0)
            assert blocked_count >= 0, enforce_summary
            ...
            blocked_rows = [
                row for row in enforce_score_ws.iter_rows(min_row=2, values_only=True)
                if row[blocked_index] == "是"
            ]
            assert bool(blocked_rows) == bool(blocked_count), (blocked_count, len(blocked_rows))
            assert all(row[blocked_index] in {"是", "否"} for row in enforce_score_ws.iter_rows(min_row=2, values_only=True))
            ...
            assert any(
                row[36] == "模型证据审查仅更新影子对比，不能单独取得100分正式出码资格"
...
def _run_job(
    ...
    job = get_job(job_id)
    assert job is not None and job["status"] in {
        "completed",
        "awaiting_confirmation",
    }, dict(job or {})
    assert (int(job["confirm_count"] or 0) > 0) == (

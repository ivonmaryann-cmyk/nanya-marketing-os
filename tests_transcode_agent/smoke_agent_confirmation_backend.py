...
def main() -> None:
    ...
    with TemporaryDirectory() as temp_dir_name:
        ...
        try:
            ...
            assert result_sheet.cell(2, 5).value == "人工已确认"
            assert result_sheet.cell(2, 6).value == "人工已确认"
            assert result_sheet.cell(2, 7).value == "人工已确认"
            ...
            assert audit_sheet.cell(2, audit_headers.index("是否长期规则") + 1).value == "是"
            assert (
                audit_sheet.cell(2, audit_headers.index("长期规则ID") + 1).value
            ...
            assert "无需人工确认".encode("utf-8") in page_response.data
            ...
            assert "确认中心长期规则".encode("utf-8") in rule_page_response.data
            ...
            assert reeval_listing["status"] == "completed", reeval_listing
            ...
            assert reeval_output.cell(2, 9).value == "可直接采用"
        finally:

...
def test_confirmed_draft_generates_agent_rules(tmp_path):
    ...
    order_text_rules = [
        ...
        if "订单" in (rule.get("条件文本", "") + rule.get("覆盖值", ""))
    ]
    assert {rule["客户简称"] for rule in order_text_rules} == {"鹤山中富", "深圳中富"}
    assert all("客户规格没有Q" in rule["条件文本"] for rule in order_text_rules)
...
def test_key_customer_rules_are_table_driven():
    ...
    assert _find_rule(rules, "惠州智恩", "grade_code", "AC", "NY-A2")
    assert _find_rule(rules, "广州名幸", "grade_code", "A1", "假板")
    assert _find_rule(rules, "赣州超跃", "grade_code", "F1", "<1oz")
    assert not _find_rule(rules, "赣州超跃", "grade_code", "F1", "铜厚:1oz")

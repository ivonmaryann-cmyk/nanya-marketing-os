...
def main() -> None:
    ...
    assert exact_grade["shadow_decision"] == "需标注"
    ...
    agent_rule = evaluate_evidence_score_shadow(
        _analysis(_field("grade", "基板级别", "AC", 100, "Agent规则覆盖", "TAR-00001")),
        semantic_evaluations=[],
        observations=_observations(),
        matrix=matrix,
    )
    assert agent_rule["shadow_score"] == 100
    assert agent_rule["shadow_decision"] == "通过"
    ...
    assert enforce["blocked"] is True
    assert enforce["effective_score"] == 94
    assert enforce["ignored_optional_missing_rules"] == 1

    print("evidence scoring smoke passed 100-point gate/default/exact/contradicted/missing verified")

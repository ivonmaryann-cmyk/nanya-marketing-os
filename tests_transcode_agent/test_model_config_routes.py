...
from fangzheng_web_app.transcode_order_semantic_model import (
    GLOBAL_ORDER_MODEL_SETTING,
    is_order_model_globally_enabled,
...
def test_global_order_model_switch_saves_and_renders(monkeypatch, tmp_path):
    client = _client(monkeypatch, tmp_path)

    response = client.post(
        "/features/transcode-agent/model-config",
        data={"action": "save_global_enabled", "enabled": "1"},
    )
    ...
    assert is_order_model_globally_enabled() is True
    body = client.get("/features/transcode-agent/model-config").get_data(as_text=True)
    assert 'name="action" value="save_global_enabled"' in body
    assert "全局启用订单备注语义模型" in body


def test_global_switch_enables_runtime_for_user(monkeypatch, tmp_path):
    ...
    db.set_setting(GLOBAL_ORDER_MODEL_SETTING, "1")

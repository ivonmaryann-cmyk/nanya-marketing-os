...
> 首次编制日期：2026-08-20  
> 当前版本：v1.10
> 当前阶段：44张业务表已在本地切换到PostgreSQL；FS-02新文件双写已完成本地验证，生产切换尚未开始
> 第一迁移模块：订单邮件自动化  
...
| 文件存储迁移 | FS-01完成；FS-02本地开发验证完成；FS-03清单工具完成；FS-04本地双读验证完成 | 本地仅新文件双写已开启；生产未开始 | 当前历史清单为0；未上传、移动或删除任何真实附件；真实对象存储待配置 |
...
- 本地订单邮件新文件已开启旧路径与`storage/automation_objects`双写，并优先读取托管副本；Git默认仍关闭双写/双读。
...
- [ ] 状态：本地开发与验证完成；生产对象存储待配置
- 负责人：Codex
- 开始时间：2026-08-21 15:25:00 +08:00
- 完成时间：2026-08-21 15:34:00 +08:00
- 当前执行边界：先完成存储无关的双写、失败隔离、对象键派生和本地托管目标验证；未配置生产对象存储，未复制历史文件
...
- 验收：断网和对象存储异常注入测试通过。
- 修改文件：`.env.example`、`fangzheng_web_app/file_storage/`、`mail_transcode_agent/mail_fetch_service.py`、`routes.py`、`tests/test_file_storage_dual_write.py`
- Git提交：`c1944fe`
- 测试结果：双写、双读、对象键越界、下载根目录、清单状态和两类故障注入5项通过；双写开启状态全套90项通过、9项按外部环境跳过
- 验证证据：27字节本地探针同时原子写入旧路径和托管路径，大小与SHA-256一致，读取优先托管副本；探针文件和空目录已清理
- 失败与回滚证据：注入托管存储异常后旧路径文件不存在，附件收集不返回可入库元数据；关闭双写/双读开关即可恢复旧路径读写，旧路径始终保留
- 遗留风险：当前托管目标是本地目录而非生产对象存储；尚未验证真实网络中断、权限、加密、版本控制和跨机恢复，因此本任务不能标记生产完成
...
- 清单命令：`python -m fangzheng_web_app.file_storage.manifest --output storage/migration_reports/file_inventory_stage_fs02.json`
- 本地清单结果：原始邮件、附件和内嵌图片合计0；已校验0、待复制0、失败0；数据库内模板版本0
- Git提交：`c1944fe`
- 迁移报告：`docs/migrations/文件存储FS02新文件双写开发报告.md`
- 当前边界：只生成清单和校验状态，未执行历史文件复制；生产历史清单必须在服务器另行生成并确认后才能复制
...
- 验证证据：本地探针优先解析到托管副本；托管副本缺失时原有`FallbackFileStorage`继续回退旧路径并记录不含正文和文件内容的日志
- 遗留风险：当前无真实历史附件样本，生产对象存储和连续7天观察未执行，不能进入FS-05
...
| 2026-08-21 | Codex | FS-02至FS-04本地开发 | 接入新邮件文件双写、对象键双读和只读历史清单 | `c1944fe` / 文档v1.10 | 本地开发验证完成 | 历史清单0项；双写探针SHA一致并已清理；故障注入通过；全套90项通过；真实对象存储、历史复制和7天观察未开始 |

...
> 首次编制日期：2026-08-20  
> 当前版本：v1.1
> 当前阶段：第一阶段开发工具已完成，实际PostgreSQL迁移尚未开始
> 第一迁移模块：订单邮件自动化  
...
|---|---|---|---|
| 第一阶段：订单邮件自动化20张表 | 迁移、核对、影子同步、回滚和观察工具已开发 | 未开始 | 未连接真实PostgreSQL，未复制数据，应用仍读写SQLite |
| 第二阶段：用户与登录 `users` | 未开始 | 未开始 | 不得因第一阶段工具完成而视为已迁移 |
| 第三阶段：PP与通用转码18张表 | 未开始 | 未开始 | `jobs`共享边界尚未梳理和迁移 |
| 第四阶段：`settings`与AI配置 | 未开始 | 未开始 | 受第二、第三阶段以及配置键域拆分前置条件阻塞 |
| 第五阶段：工作规划与反馈 | 未开始 | 未开始 | 等前序模块稳定后实施 |
| 文件存储迁移 | FS-01本地抽象已开发，其余未开始 | 未开始 | 未上传、移动或删除任何真实附件 |

当前生产/本地业务事实：

- `AUTOMATION_DATABASE_BACKEND=sqlite`；
- `AUTOMATION_POSTGRESQL_READ_WRITE_ENABLED=false`；
- 没有业务表正式切换到PostgreSQL；
- 没有执行生产数据回放、正式切换或7至14天稳定观察；
- SQLite历史数据和本地附件仍是现有业务数据源，禁止删除。
...
前置阻塞：第9.1用户与登录、第9.2 PP与通用转码尚未迁移，混合键不得正式切换。2026-08-20曾短暂标记“进行中”，尚未产生代码或数据迁移，现已按实际情况纠正。
...
1. 准备隔离的PostgreSQL开发和测试实例，不使用生产连接；
2. 执行`0001_automation_schema.sql`至`0003_cutover_rollback.sql`并验证结构、权限和默认关闭开关；
3. 使用代表性SQLite快照执行20张表全量复制、重复执行、全量核对和测试回滚；
4. 验证PostgreSQL断线期间SQLite业务正常、恢复后Outbox积压可自动追平；
5. 执行已批准阈值下的性能与并发测试、备份恢复演练和业务人工验收；
6. 连续至少7天影子比较无未解释差异；

...
> 首次编制日期：2026-08-20  
> 当前版本：v1.4
> 当前阶段：第一阶段已在本地切换到PostgreSQL，生产切换尚未开始
> 第一迁移模块：订单邮件自动化  
...
- 本地未跟踪的`config/local.env`已设置`AUTOMATION_DATABASE_BACKEND=postgresql`和`AUTOMATION_POSTGRESQL_READ_WRITE_ENABLED=true`；
- 本地PostgreSQL的20张业务表均已启用变更捕获，切换后新增、修改和删除可进入回滚日志；
- Git中的安全默认值仍为SQLite且PostgreSQL读写关闭；
- 第一阶段20表仅在本地正式读写PostgreSQL，生产没有切换；
...
- 本地自检补充：`capture_changes=true`；20张业务表触发器均存在；真实应用新增和删除已写入2条未回放变更日志。该证据只验证捕获链路，不等同于完整回滚演练

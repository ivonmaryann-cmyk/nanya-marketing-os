...
> 首次编制日期：2026-08-20  
> 当前版本：v1.7
> 当前阶段：第一至第三阶段已在本地切换到PostgreSQL；生产切换尚未开始
> 第一迁移模块：订单邮件自动化  
...
- 本地未跟踪的`config/local.env`已分别启用自动化、身份和转码PostgreSQL后端；
- 本地PostgreSQL的20张业务表均已启用变更捕获，切换后新增、修改和删除可进入回滚日志；
- Git中的安全默认值仍为SQLite且PostgreSQL读写关闭；
- 第一至第三阶段共39张业务表仅在本地正式读写PostgreSQL，生产没有切换；
- 没有执行生产数据回放、正式切换或7至14天稳定观察；
- SQLite历史数据继续保留为回滚源；未迁移模块仍使用SQLite；本地任务文件和附件仍按原路径读取，禁止删除。

...
MIGRATION_DIR = Path(__file__).resolve().parents[2] / "migrations" / "automation" / "postgresql"
...
def apply_migrations(connection) -> list[str]:
    ...
    for path in sorted(MIGRATION_DIR.glob("*.sql")):
        ...
        row = connection.execute(
            "SELECT checksum FROM automation_schema_migrations WHERE version=%s", (path.name,)
        ).fetchone()
        if row:
        ...
        connection.execute(
            "INSERT INTO automation_schema_migrations(version, checksum) VALUES (%s, %s)",
            (path.name, checksum),
        )

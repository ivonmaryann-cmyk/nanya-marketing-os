...
RULES = [
    ...
    ("pp", "PP厚度 / RC", "按单位区分：百分数表示含量，乘10生成3位码（如40%=400）；μm表示厚度，仍限小于300且末位0/3/5/8。未确认厚度不自动放行。"),
    ("roll", "卷料尺寸", "R+3位卷长米数+4位幅宽编码；未注明卷长用000，300M用300。49.7英寸=4970，43.8英寸=4380，49英寸=XXXX。X是固定字符；未注明幅宽不猜测。"),
    ("copper", "铜箔四位码", "前2位为两面铜箔规格，后2位为两面处理类型。Hoz=H，HTE=N，RTF3=G；Hoz HTE=HHNN，1oz/Hoz RTF3=H1GG。其他不同铜重的排序需业务确认。"),
]

# Business-confirmed additions; existing manually maintained records are preserved.
SEEDS["copper_weight"]["H"] = "Hoz"
SEEDS["copper_pair"].update({"HHNN": "双面Hoz HTE", "H1GG": "1oz/Hoz RTF3（业务确认组合）"})
SEEDS["board_spec"].update({"C3": "芯厚 C3", "T3": "总厚 T3", "C2": "芯厚 C2", "T2": "总厚 T2"})
SEEDS["roll_size"].update({"R0004970": "幅宽49.7英寸（卷长未注明）", "R0004380": "幅宽43.8英寸（卷长未注明）", "R000XXXX": "幅宽49英寸（卷长未注明）"})

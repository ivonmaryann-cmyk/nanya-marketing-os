...
LOG = logging.getLogger("configuration_migration")
    ...
    with sqlite_snapshot(sqlite_path) as snapshot, psycopg.connect(
        ...
        with target.transaction():
            ...
            if not verification["ok"]:
                raise RuntimeError("configuration verification failed; PostgreSQL transaction rolled back")
...
def _configure_logging() -> None:
    log_path = Path(
        os.getenv(
            ...
            "storage/migration_logs/configuration_migration.log",
        )
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Migrate only global settings and PDF/Excel AI configuration")
    ...
    try:
        if args.command == "migrate":
            ...
            if args.confirm != "DROP-CONFIGURATION-TEST-DATA":
                raise RuntimeError("test rollback confirmation does not match")
            with psycopg.connect(
                _url("CONFIG_TEST_DATABASE_URL"), row_factory=dict_row
            ) as target, target.transaction():
                target.execute("DROP TABLE IF EXISTS pdf_excel_ai_config_versions CASCADE")
                target.execute("DROP TABLE IF EXISTS settings CASCADE")
                target.execute("DROP TABLE IF EXISTS configuration_change_log CASCADE")
                target.execute("DROP TABLE IF EXISTS configuration_runtime_flags CASCADE")
                target.execute("DROP TABLE IF EXISTS configuration_schema_migrations CASCADE")
                target.execute("DROP FUNCTION IF EXISTS configuration_capture_change() CASCADE")
            result = {"ok": True, "scope": "configuration test target"}
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0 if result.get("ok", result.get("verification", {}).get("ok", True)) else 2
    except Exception as exc:
        LOG.error(
            "configuration migration command failed (%s); sensitive details were omitted",
            type(exc).__name__,
        )
        return 1

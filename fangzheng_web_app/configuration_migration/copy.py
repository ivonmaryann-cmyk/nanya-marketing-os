...
from .spec import PRIMARY_KEYS, TABLE_COLUMNS, TABLES
...
def _upsert_sql(table: str) -> str:
    ...
    return f"INSERT INTO {_quote(table)}({quoted}) VALUES ({placeholders}) ON CONFLICT ({conflict}) DO UPDATE SET {updates}"
...
def copy_snapshot(snapshot: Path, target: Any) -> dict[str, int]:
    ...
    with closing(sqlite3.connect(snapshot)) as source:
        ...
        with target.cursor() as cursor:
            for table in TABLES:
                ...
                columns = ",".join(_quote(column) for column in TABLE_COLUMNS[table])
                order = ",".join(_quote(column) for column in PRIMARY_KEYS[table])
                rows = source.execute(f'SELECT {columns} FROM "{table}" ORDER BY {order}').fetchall()
                if rows:
                    cursor.executemany(
                        _upsert_sql(table),
    ...
    target.execute(
        "SELECT setval(pg_get_serial_sequence('pdf_excel_ai_config_versions', 'id'), "
        "GREATEST(COALESCE((SELECT MAX(id) FROM pdf_excel_ai_config_versions), 0), 1), "
        "EXISTS(SELECT 1 FROM pdf_excel_ai_config_versions))"
    )
    return counts

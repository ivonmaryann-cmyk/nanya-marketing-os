...
ENABLE_CONFIRMATION = "ENABLE-CONFIGURATION-CHANGE-CAPTURE"
ROLLBACK_CONFIRMATION = "REPLAY-CONFIGURATION-CHANGES-TO-SQLITE"
...
def _verify_backup(path: Path, expected_hash: str) -> None:
    if not path.is_file() or file_sha256(path) != expected_hash:
        raise RuntimeError("configuration SQLite backup is missing or changed")
...
def enable_change_capture(database_url: str, backup_path: Path, backup_sha256: str, confirmation: str) -> None:
    if confirmation != ENABLE_CONFIRMATION:
        raise RuntimeError("configuration capture confirmation does not match")
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as target, target.transaction():
        cursor = target.execute(
            "UPDATE configuration_runtime_flags SET value='true',updated_at=CURRENT_TIMESTAMP "
            "WHERE key='capture_changes' AND value='false'"
        )
        if cursor.rowcount != 1:
            raise RuntimeError("configuration change capture was already enabled or is unavailable")
...
def apply_changes_to_sqlite(connection: sqlite3.Connection, changes: list[dict[str, Any]]) -> int:
    ...
    for change in changes:
        ...
        if table not in TABLE_COLUMNS:
            raise ValueError("configuration change table is outside the migration scope")
        ...
        if not isinstance(pk, dict) or set(pk) != set(keys):
            raise ValueError("configuration change primary key schema mismatch")
        ...
        if operation == "delete":
            ...
            if not isinstance(row, dict) or set(row) != set(columns):
                raise ValueError("configuration change row schema mismatch")
            ...
            raise ValueError(f"unsupported configuration change operation: {operation}")
        applied += 1
    return applied
...
def replay_change_log(
    ...
    *,
    ...
    if confirmation != ROLLBACK_CONFIRMATION:
        raise RuntimeError("configuration rollback confirmation does not match")
    _verify_backup(backup_path, backup_sha256)
    ...
    with psycopg.connect(database_url, row_factory=dict_row) as target:
        changes = [dict(row) for row in target.execute(
            "SELECT id,table_name,operation,pk_json,row_json FROM configuration_change_log "
        ...
        with target.transaction():
            target.execute(
                "UPDATE configuration_change_log SET replayed_at=CURRENT_TIMESTAMP WHERE id=ANY(%s)",
                ([change["id"] for change in changes],),
            )
            remaining = int(target.execute(
                "SELECT COUNT(*) AS total FROM configuration_change_log WHERE replayed_at IS NULL"
            ).fetchone()["total"])
            if remaining == 0:
                target.execute(
                    "UPDATE configuration_runtime_flags SET value='false',updated_at=CURRENT_TIMESTAMP "

...
def verify_snapshot(snapshot: Path, target: Any) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(snapshot)) as source:
        ...
        orphan_count = source.execute(
            "SELECT COUNT(*) FROM personal_tasks t LEFT JOIN task_categories c ON c.id=t.category_id "
            "WHERE t.category_id IS NOT NULL AND c.id IS NULL"
        ).fetchone()[0]
        report["task_category_orphans"] = int(orphan_count)
        report["sqlite_integrity"] = source.execute("PRAGMA integrity_check").fetchone()[0]
        report["ok"] = report["ok"] and orphan_count == 0 and report["sqlite_integrity"] == "ok"
    return report
